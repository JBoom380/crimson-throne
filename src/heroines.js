...
(function () {
  ...
  Field.prototype.near = function (x, y, z, r) {
    const L = [], S = [], lim = r + this.maxOff + 0.01;
    for (const P of this.prims) if (bbDist(P.bb, x, y, z) < lim + P.k) (P.sub ? S : L).push(P);
    return { L, S };
  };
  Field.prototype.all = function () { return this.near(0, 0, 0, 1e9); };
  Field.prototype.eval = function (x, y, z, nl) {
    ...
    if (this.masks.length) for (let gi = 0; gi < ng; gi++) {
      if (g[gi] > 0.05) continue;
  ...
  Field.prototype.at = function (x, y, z) { return this.eval(x, y, z, this.near(x, y, z, 0.06)); };
  ...
  function polygonize(F, bnd, h, smoothIt) {
    ...
    for (const bid of near) {
      ...
      for (let k = bk * B; k <= k1; k++) for (let j = bj * B; j <= j1; j++) for (let i = bi * B; i <= i1; i++)
        val[i + nx * j + nxy * k] = F.eval(x0 + i * h, y0 + j * h, z0 + k * h, nl);
    }
    // vertices
    ...
    for (let v = 0; v < nv; v++) {
      ...
      for (let it = 0; it < 2; it++) {
        const d = F.eval(x, y, z, nl); const g = F.grad(x, y, z, nl, e);
        const s = clamp(d, -h, h); x -= g[0] * s; y -= g[1] * s; z -= g[2] * s;
      }
      const g = F.grad(x, y, z, nl, e);
  ...
  const U = { uT: { value: 0 }, uRimCol: { value: new T.Color(1.0, 0.42, 0.30) }, uRimK: { value: 0.9 }, uWarm: { value: new T.Vector3(0.16, 0.05, 0.035) } };
  const IDLE = `
  ...
  const PATTERN = `
    ...
    vec3 ctPat(vec3 c, float pid, vec3 o, float sd){
      ...
      } else if (pid < 5.5) {                  // cloth
        c *= 0.88 + 0.16*ctN(vec2(o.x*90.0 + o.z*90.0, o.y*16.0));
        ...
        sh.fragmentShader = sh.fragmentShader
          ...
          .replace('#include <opaque_fragment>', `
            { vec3 N = normalize(normal); vec3 V = normalize(vViewPosition); if (dot(N, V) < 0.0) N = -N; float ndv = clamp(dot(N, V), 0.0, 1.0);
                ...
                float sp = smoothstep(0.80, 0.83, nh) * 0.22 + smoothstep(0.93, 0.945, nh) * 0.55;
                outgoingLight += directionalLights[0].color * sp * gloss * 0.55;
              #endif
              float fr = pow(1.0 - ndv, 2.2);
              outgoingLight += uRimCol * smoothstep(0.35, 0.6, fr) * uRimK * (0.35 + 0.4 * gloss) * (0.4 + 0.6 * max(-N.z*0.0 + 1.0 - ndv, 0.0));
            }
  ...
  function makeRig(P) {
    const r = {};
    r.pel = { o: P.pel, R: eul(P.pelYaw, P.pelPitch, P.pelRoll) };
    const wR = mm(r.pel.R, eul(P.wYaw, P.wPitch, P.wRoll));
    r.wst = { o: W(r.pel, [0, 0.175, -0.012]), R: wR };
    const cR = mm(wR, eul(P.cYaw, P.cPitch, P.cRoll));
    r.chs = { o: W(r.wst, [0, 0.2, 0.0]), R: cR };
    ...
    r.leg = {}; r.arm = {};
    for (const s of [-1, 1]) {
      const hip = W(r.pel, [s * 0.088, -0.004, 0.004]);
      const k = ik(hip, P.ank[s], 0.44, 0.42, P.knee[s]);
  ...
  function bodyField(rig, S) {
    const F = new Field(5, [0, 0.03, 0.03, 0.04, 0.04]);
    const { pel, wst, chs } = rig; const b = S.bust || 1, hp = S.hip || 1;
    F.join(0, 0, 0.05); F.join(1, 0, 0.024); // breasts pair, glutes pair
    // pelvis + hips
    F.ell(0, pel, [0, 0.05, -0.008], [0.15 * hp, 0.118, 0.1], 0);
    F.ell(0, pel, [-0.13 * hp, -0.012, -0.006], [0.08, 0.105, 0.086], 0.05);
    F.ell(0, pel, [0.13 * hp, -0.012, -0.006], [0.08, 0.105, 0.086], 0.05);
    F.ell(0, pel, [0, 0.08, 0.05], [0.105, 0.075, 0.058], 0.04);                         // lower belly
    F.ell(0, pel, [0, -0.052, 0.042], [0.05, 0.046, 0.04], 0.03);                         // groin
    for (const s of [-1, 1]) {
      F.ell(0, pel, [s * 0.074 * hp, -0.022, -0.078], [0.098 * hp, 0.106, 0.092], 0, { j: 1, rot: eul(s * 0.25, 0.2, 0) }); // glutes
      F.ell(0, pel, [s * 0.092, 0.105, 0.068], [0.018, 0.02, 0.018], 0.028);             // hip bone
    }
    // waist + abdomen
    F.ell(0, wst, [0, -0.005, 0], [0.104 * (S.waist || 1), 0.13, 0.08], 0.085);
    F.ell(0, wst, [0, 0.02, 0.045], [0.068, 0.1, 0.042], 0.04);
    ...
    for (const s of [-1, 1]) {
      ...
      const bc = [s * 0.08, 0.0 + (S.bustY || 0), 0.088];
      F.ell(0, chs, bc, [0.078 * b, 0.073 * b, 0.07 * b], 0, { j: 0, rot: eul(s * 0.32, -0.12, 0) }); // breasts
      F.cone(0, W(chs, [s * 0.022, 0.168, 0.055]), W(chs, [s * 0.148, 0.176, 0.008]), 0.011, 0.012, 0.022); // collarbone
      F.cone(0, W(chs, [s * 0.04, 0.19, -0.03]), W(chs, [s * 0.15, 0.162, -0.018]), 0.03, 0.026, 0.04);    // trapezius
    }
    F.ell(0, chs, [0, 0.13, -0.014], [0.125, 0.055, 0.078], 0.05);                          // shoulder girdle
    ...
    for (const s of [-1, 1]) {
      ...
      const th = S.thigh || 1;
      F.cone(g, K, H, 0.054, 0.096 * th, 0);
      F.ell(g, { o: lerp3(K, H, 0.55), R: basisY(up, fwd) }, [s * 0.03, 0, 0.004], [0.056 * th, 0.15, 0.058 * th], 0.02);   // outer thigh
      ...
      F.ell(g, { o: lerp3(K, H, 0.58), R: basisY(up, fwd) }, [0, 0, -0.03], [0.055 * th, 0.15, 0.05], 0.02);              // hamstrings
    ...
    for (const s of [-1, 1]) {
      ...
      F.ell(g, { o: Sh, R: chs.R }, [s * 0.018, 0.004, -0.004], [0.05, 0.062, 0.056], 0.03);                 // deltoid
      F.cone(g, E, Sh, 0.033, 0.045, 0.02);
      ...
      F.cone(g, Wr, E, 0.02, 0.032, 0.015, { sx: 1.2, sz: 0.85, zh: hint });
      F.ell(g, frF, [0, lf * 0.72, 0.004], [0.034, 0.07, 0.028], 0.025);                                  // forearm muscle
  ...
  function headField(S) {
    ...
    F.ell(0, O, [0, -0.064, 0.02], [0.049 * jw, 0.034, 0.058], 0.03);          // jaw
    for (const s of [-1, 1]) {
      F.ell(0, O, [s * 0.047 * jw, -0.055, -0.012], [0.016, 0.026, 0.027], 0.03);  // jaw angle
    ...
    F.ell(0, O, [0, -0.092, 0.056], [0.021 * (S.chin || 1), 0.018, 0.019], 0.025); // chin
    ...
    const lf = S.lips || 1;
    F.ell(0, O, [0, -0.046, 0.082], [0.021, 0.0072 * lf, 0.011], 0.012);           // upper lip
    F.ell(0, O, [0, -0.058, 0.08], [0.018, 0.0085 * lf, 0.0115 * lf], 0.012);      // lower lip
    ...
    F.ell(0, O, [0, -0.052, 0.094], [0.02, 0.0022, 0.012], 0.004, { sub: true });  // mouth line
    return F;
  }
  ...
  function paintFace(S) {
    ...
    // eyes
    for (const s of [-1, 1]) {
      const cx = FU(s * 0.031), cy = FV(0.003), w = 34, hgt = 13 * (S.eyeOpen || 1), tilt = (S.eyeTilt || 0.1) * -s;
      g.save(); g.translate(cx, cy); g.rotate(tilt);
      g.fillStyle = '#e9ddd6'; g.beginPath(); g.moveTo(-w, 2); g.quadraticCurveTo(-w * 0.2, -hgt * 1.6, w, -hgt * 0.35); g.quadraticCurveTo(w * 0.1, hgt * 1.3, -w, 2); g.fill();
      g.fillStyle = S.iris; g.beginPath(); g.arc(-s * 2, -1, hgt * 0.95, 0, PI * 2); g.fill();
      ...
      g.lineTo(w + 16, -hgt * 1.1); g.lineTo(w + 5, -hgt * 0.1); g.quadraticCurveTo(-w * 0.2, -hgt * 1.25, -w, 2); g.fill();
      if (S.lidLow) { g.globalAlpha = 0.9; g.fillStyle = S.lidCol || '#3a2030'; g.beginPath(); g.moveTo(-w, 0); g.quadraticCurveTo(0, -hgt * 1.9, w + 2, -hgt * 0.4); g.quadraticCurveTo(0, -hgt * S.lidLow, -w, 0); g.fill(); g.globalAlpha = 1; }
      ...
      g.moveTo(40, 6 + ang * 14); g.quadraticCurveTo(0, -14 - (S.browArch || 0) * 10, -40, 2); g.lineTo(-38, 8); g.quadraticCurveTo(0, -4 - (S.browArch || 0) * 10, 38, 14 + ang * 14); g.fill();
      g.restore();
    }
    // nose shading
    g.fillStyle = 'rgba(90,30,20,0.35)'; for (const s of [-1, 1]) { g.beginPath(); g.ellipse(FU(s * 0.0085), FV(-0.03), 6, 4, 0, 0, PI * 2); g.fill(); }
    // lips
    const ly = FV(-0.052), lx = FU(0), lw = 60 * (S.mouthW || 1), grin = S.grin || 0;
  ...
  const DEF = {};
  DEF.kaela = {
    skin: '#c48f60', skinGloss: 0.85, hair: '#161012', phase: 0,
    shape: { bust: 1.04, hip: 1.03, thigh: 1.05, calf: 1.05, fist: { '-1': true, '1': true } },
    pose: {
      pel: [-0.07, 0.912, 0.0], pelYaw: 0.1, pelPitch: 0.2, pelRoll: -0.15,
      wYaw: -0.05, wPitch: -0.12, wRoll: 0.09, cYaw: -0.1, cPitch: -0.14, cRoll: 0.09,
      hYaw: 0.1, hPitch: -0.1, hRoll: -0.05,
      ank: { '-1': [-0.1, 0.072, -0.01], '1': [0.14, 0.14, 0.13] }, knee: { '-1': [0, 0, 1], '1': [-0.2, 0, 1] },
      fYaw: { '-1': -0.2, '1': 0.45 }, fPitch: { '-1': 0, '1': 0.55 },
      wr: { '-1': [-0.05, 1.035, 0.34], '1': [0.05, 1.055, 0.33] }, elb: { '-1': [-1, -0.2, -0.3], '1': [1, -0.2, -0.3] },
      hd: { '-1': [0.35, -0.45, 0.8], '1': [-0.35, -0.4, 0.8] }, hf: { '-1': [0, 1, 0], '1': [0, 1, 0] },
    },
    face: { iris: '#5a3212', lid: 'rgba(60,30,20,0.9)', blush: 'rgba(170,70,50,0.35)', brow: '#140a08', browAng: 0.9, browArch: 0.2, lips: '#9a1e1c', eyeTilt: 0.14, eyeOpen: 0.9, scar: true, lidLow: 0.9 },
    head: { jaw: 1.06, chin: 1.1, lips: 1.1 },
  };
  DEF.nyx = {
    skin: '#ecdcd8', skinGloss: 0.7, hair: '#dfe3f0', phase: 2.1,
    shape: { bust: 1.02, hip: 1.0, thigh: 1.0, calf: 0.95, waist: 0.96, fist: { '-1': true, '1': false } },
    pose: {
      pel: [0.035, 0.915, 0.0], pelYaw: -0.22, pelPitch: 0.2, pelRoll: 0.12,
      wYaw: 0.08, wPitch: -0.14, wRoll: -0.06, cYaw: 0.14, cPitch: -0.16, cRoll: -0.1,
      hYaw: -0.1, hPitch: -0.12, hRoll: 0.12,
      ank: { '-1': [-0.2, 0.11, 0.22], '1': [0.085, 0.072, -0.02] }, knee: { '-1': [0.3, 0, 1], '1': [0, 0, 1] },
      fYaw: { '-1': -0.25, '1': 0.2 }, fPitch: { '-1': 0.45, '1': 0 },
      wr: { '-1': [-0.39, 1.78, 0.1], '1': [0.2, 0.99, 0.04] }, elb: { '-1': [-1, -0.4, -0.2], '1': [1, 0.1, -0.7] },
      hd: { '-1': [0.1, 1, 0.05], '1': [-0.2, -0.8, 0.45] }, hf: { '-1': [1, 0, 0], '1': [0.8, 0, 0.3] },
    },
    face: { iris: '#8a4ae8', lid: 'rgba(70,30,80,0.95)', lidCol: '#3a1c40', blush: 'rgba(170,80,110,0.25)', brow: '#6a6070', browAng: -0.1, browArch: 0.9, lips: '#6e0f2e', eyeTilt: 0.2, eyeOpen: 0.85, lidLow: 0.2 },
    head: { jaw: 0.97, chin: 1.0, lips: 1.15 },
  };
  DEF.vesna = {
    skin: '#e3a986', skinGloss: 0.8, hair: '#c4401a', phase: 4.2,
    shape: { bust: 0.98, hip: 1.02, thigh: 1.02, calf: 1.05, fist: { '-1': true, '1': false } },
    pose: {
      pel: [0.0, 0.915, 0.0], pelYaw: 2.35, pelPitch: 0.17, pelRoll: 0.1,
      wYaw: -0.22, wPitch: -0.08, wRoll: -0.05, cYaw: -0.3, cPitch: -0.1, cRoll: -0.08,
      hYaw: -1.3, hPitch: 0.06, hRoll: 0.1,
      ank: { '-1': [0.2, 0.1, -0.03], '1': [-0.06, 0.072, -0.09] }, knee: { '-1': [0.6, 0, -0.8], '1': [0.7, 0, -0.7] },
      fYaw: { '-1': 2.2, '1': 2.5 }, fPitch: { '-1': 0.35, '1': 0 },
  ...
  function outfit(id, F, rig, D) {
    ...
    const cupMask = (s, rC, neck) => MK.and(
      MK.ell(chs, [s * 0.082, -0.012 + (D.shape.bustY || 0), 0.09], [rC, rC * 0.97, rC * 0.92]),
      MK.half(chs, q => (-0.012 + neck) + (s * q[0] - 0.05) * 0.65 - q[1]));
    if (id === 'kaela') {
      const bronze = hx('#b27a2c');
      for (const s of [-1, 1]) {
        F.mask(TOR, cupMask(s, 0.083, 0.052), 0.007, { col: bronze, pid: 1, gloss: 1.0 });
        F.mask(TOR, MK.cap(W(chs, [s * 0.11, 0.05, 0.07]), W(chs, [s * 0.1, 0.19, -0.02]), 0.011), 0.004, { col: hx('#6a4012'), pid: 2, gloss: 0.3 });
      }
      ...
      F.mask(TOR | LEGS, MK.or(MK.ell(pel, [0, -0.035, 0.055], [0.075, 0.1, 0.075]), MK.ell(pel, [0, -0.075, 0], [0.04, 0.05, 0.12]), MK.ell(pel, [0, -0.025, -0.095], [0.045, 0.11, 0.07])), 0.004, { col: hx('#7a1014'), pid: 5, gloss: 0.2 });
      ...
      const L = rig.leg[1];
      const sa = lerp3(L.knee, L.hip, 0.45), sb = lerp3(L.knee, L.hip, 0.62);
      F.paint = (x, y, z, c) => { const d = MK.cap(add(sa, [0.035, 0, 0.05]), add(sb, [0.055, 0.0, 0.03]), 0.006)(x, y, z); return d > 0 ? mixc(c, hx('#8a3a30'), 0.7) : c; };
    }
    if (id === 'nyx') {
      const indigo = hx('#2c1f62'), silver = hx('#c9d0e4');
      for (const s of [-1, 1]) {
        F.mask(TOR, MK.or(MK.and(MK.ell(chs, [s * 0.082, -0.012, 0.09], [0.088, 0.1, 0.09]), MK.half(chs, q => s * q[0] - (0.008 + (q[1] + 0.08) * 0.14))),
          MK.cap(W(chs, [s * 0.08, 0.06, 0.08]), W(chs, [s * 0.03, 0.2, -0.03]), 0.016)), 0.006, { col: indigo, pid: 5, gloss: 0.5 });
      }
      ...
      F.mask(TOR | LEGS, MK.or(MK.ell(pel, [0, -0.035, 0.055], [0.075, 0.1, 0.075]), MK.ell(pel, [0, -0.075, 0], [0.04, 0.05, 0.12]), MK.ell(pel, [0, -0.025, -0.095], [0.05, 0.11, 0.07])), 0.004, { col: hx('#1c1440'), pid: 5, gloss: 0.3 });
      for (const s of [-1, 1]) {
        const L = rig.leg[s], up = nrm(sub(L.knee, L.ank)), fr = { o: L.ank, R: basisY(up, [0, 0, 1]) };
    ...
    if (id === 'vesna') {
      const fur = hx('#9a7448'), leather = hx('#5a3218');
      for (const s of [-1, 1]) {
        F.mask(TOR, cupMask(s, 0.082, 0.06), 0.012, { col: fur, pid: 3, gloss: 0.1 });
        F.mask(TOR, MK.cap(W(chs, [s * 0.1, 0.055, 0.07]), W(chs, [s * 0.08, 0.19, -0.03]), 0.01), 0.004, { col: leather, pid: 2, gloss: 0.3 });
      }
      ...
      F.mask(TOR | LEGS, MK.or(MK.ell(pel, [0, -0.035, 0.055], [0.075, 0.1, 0.075]), MK.ell(pel, [0, -0.075, 0], [0.04, 0.05, 0.12]), MK.ell(pel, [0, -0.025, -0.095], [0.045, 0.11, 0.07])), 0.005, { col: hx('#6a4a2c'), pid: 3, gloss: 0.1 });
      ...
      const L = rig.leg[-1];
      const segs = [0.4, 0.52, 0.64].map(t => [lerp3(L.knee, L.hip, t), lerp3(L.knee, L.hip, t + 0.07)]);
      F.paint = (x, y, z, c) => {
        for (const [a, b] of segs) { const d = MK.cap(a, b, 0.07)(x, y, z); if (d > 0) { const q = sub([x, y, z], a); const side = dot(nrm(q), WD(rig.pel, [-1, 0, 0.3])); if (side > 0.45) return mixc(c, hx('#a0101a'), 0.85); } }
        return c;
      };
    }
  }
  ...
  function extras(id, D, F, HF, rig, cols) {
    ...
    if (id === 'kaela') {
      ...
      gridMesh(Sft, drape(bodyAt, beltPts(0.045, 0.3, -0.085, 0.085, false, 7), 0.42, 14, { clr: 0.014, taper: 0.8, bias: [0, 0, 0.06] }), edgeCol(crimson, crD, 0.9), 5, 0.2, v => v * v * 0.8);
      gridMesh(Sft, drape(bodyAt, beltPts(0.1, 0.3, -0.09, 0.09, true, 7), 0.45, 14, { clr: 0.014, taper: 0.85, bias: [0, 0, -0.05] }), edgeCol(crimson, crD, 0.9), 5, 0.2, v => v * v * 0.8);
      // greatsword planted point-down
      const tip = [0, 0.0, 0.56], top = [0, 1.0, 0.41], ax = nrm(sub(top, tip)), R = basisY(ax, [1, 0, 0]);
      ...
      const capeTop = []; for (let i = 0; i < 8; i++) { const t = i / 7; capeTop.push(add(lerp3(W(chs, [-0.19, 0.17, -0.02]), W(chs, [0.02, 0.2, -0.1]), t), WD(chs, [0, 0, -0.03 - 0.03 * Math.sin(t * PI)]))); }
      const fur = hx('#6e6050'), furD = hx('#3e342c');
      gridMesh(Sft, drape(bodyAt, capeTop, 1.0, 22, { clr: 0.022, taper: 1.2, bias: [-0.02, 0, -0.06], stiff: 0.8, folds: [0.03, 2.5] }), (u, v) => mixc(fur, furD, (hash3(u * 9, v * 30, 1) * 0.5 + (v > 0.93 ? 0.5 : 0))), 3, 0.1, v => v * 0.9);
    }
    if (id === 'nyx') {
      const ind = hx('#2e2066'), indD = hx('#140c30');
      gridMesh(Sft, drape(bodyAt, beltPts(0.04, 0.25, -0.1, 0.1, false, 8), 0.84, 26, { clr: 0.013, taper: 1.2, bias: [0, 0, 0.03] }), edgeCol(ind, indD, 0.95), 5, 0.4, v => v * v);
      gridMesh(Sft, drape(bodyAt, beltPts(0.1, 0.25, -0.11, 0.11, true, 8), 0.9, 26, { clr: 0.013, taper: 1.25, bias: [0, 0, -0.04] }), edgeCol(ind, indD, 0.95), 5, 0.4, v => v * v);
      // staff + moon orb
      const bot = [-0.46, 0.0, 0.13], tp = [-0.4, 2.02, 0.1];
    ...
    if (id === 'vesna') {
      const fur = hx('#94703f'), furD = hx('#4a3418');
      const furCol = (u, v) => mixc(fur, furD, hash3(u * 13, v * 17, 3) * 0.5 + (v > 0.85 ? 0.35 : 0));
      gridMesh(Sft, drape(bodyAt, beltPts(0.035, 0.28, -0.085, 0.085, false, 6), 0.24, 9, { clr: 0.016, taper: 0.75, bias: [0, 0, 0.05] }), furCol, 3, 0.1, v => v * 0.7);
      gridMesh(Sft, drape(bodyAt, beltPts(0.09, 0.28, -0.085, 0.085, true, 6), 0.25, 9, { clr: 0.016, taper: 0.75, bias: [0, 0, -0.06] }), furCol, 3, 0.1, v => v * 0.7);
  ...
  function hairField(id) {
    ...
    F.ell(0, O, [0, 0.034, -0.018], [0.079, 0.1, 0.101], 0);                 // cap
    F.ell(0, O, [0, 0.082, 0.012], [0.07, 0.05, 0.07], 0.03);                // crown volume
    ...
    if (id === 'kaela') {
      lobes(7, 1.9, 4.4, 0.075, 0.03, 0.03, [0.042, 0.075, 0.04], 0.035);
      lobes(5, 2.3, 4.0, 0.065, -0.06, 0.02, [0.04, 0.06, 0.038], 0.03);
  ...
  function hairStrands(id, D, rig, F, HF, Sft) {
    const hd = rig.head, col = hx(D.hair), colD = mixc(col, [0, 0, 0], 0.45), colL = mixc(col, [1, 1, 1], 0.25);
    const both = p => { const q = toL(hd, p[0], p[1], p[2]); return Math.min(F.at(p[0], p[1], p[2]), HF.at(q[0], q[1], q[2])); };
    const braid = (start, lenB, r0, sway, clr, bias) => {
      ...
      tube(Sft, pts, t => r0 * (1 - 0.45 * t) * (0.72 + 0.28 * Math.abs(Math.sin(t * 60))), t => mixc(col, colD, 0.5 * Math.abs(Math.cos(t * 60))), 7, 0.6, t => t * sway, 7);
      tube(Sft, pts.slice(-4).concat([add(pts[pts.length - 1], scl(nrm(sub(pts[pts.length - 1], pts[pts.length - 2])), 0.06))]), t => r0 * 0.5 * (1 - t * 0.7), colD, 7, 0.4, () => sway, 6);
      return path;
    };
    if (id === 'kaela') braid(W(hd, [0, -0.045, -0.115]), 0.72, 0.026, 0.5, 0.03, [0, 0, -0.02]);
    if (id === 'vesna') braid(W(hd, [0.06, -0.06, -0.09]), 0.55, 0.032, 0.6, 0.035, [0.02, 0, 0.1]);
    if (id === 'nyx' || id === 'vesna') {
      const n = id === 'nyx' ? 22 : 16, L0 = id === 'nyx' ? 0.8 : 0.36;
      for (let i = 0; i < n; i++) {
        const a = -2.1 + 4.2 * i / (n - 1), rr = 0.095;
        const base = W(hd, [Math.sin(a) * rr, -0.01 - Math.abs(Math.sin(a)) * 0.02, Math.cos(a) * rr * 0.6 - 0.035 - (id === 'nyx' ? 0.0 : 0.02)]);
        if (Math.cos(a) > 0.45) continue;
        const side = WD(hd, [Math.cos(a), 0, -Math.sin(a)]), w = id === 'nyx' ? 0.034 : 0.04;
        const top = [add(base, scl(side, -w / 2)), add(base, scl(side, w / 2))];
        const lenS = L0 * (0.8 + 0.4 * hash3(i, 1, 2)) * (id === 'nyx' ? (1 - 0.25 * Math.abs(Math.sin(a))) : 1);
        const g = drape(both, top, lenS, id === 'nyx' ? 20 : 10, { clr: 0.01 + 0.008 * hash3(i, 3, 4), bias: [Math.sin(a) * 0.12 * (id === 'vesna' ? 2 : 1), 0, -0.06], taper: id === 'nyx' ? 0.6 : 0.3, stiff: 0.75 });
        gridMesh(Sft, g, (u, v) => mixc(mixc(col, colL, 0.35 * Math.abs(Math.sin((u + i) * 3))), colD, v * 0.35), 7, 0.6, v => v * 0.8);
      }
    }
  }
  ...
  function bake(id) {
    ...
    outfit(id, F, rig, D);
    const body = bakeBody(F, rig, D, 0.011);
    const HFs = headField(D.head), skin = hx(D.skin);
    const head = bakeLocal(HFs, rig.head, 0.0042, (l, n) => ({ c0: skin, c1: [sstep(0.0, 0.3, n[2]) * sstep(-0.13, -0.1, l[1]), 0, 0], cl: [-1, 0, 0, 0] }), l => [0.5 + l[0] / 0.2, 0.5 + (l[1] + 0.02) / 0.2]);
    const HR = hairField(id), hc = hx(D.hair);
    const hair = bakeLocal(HR, rig.head, 0.008, (l, n) => ({ c0: hc, c1: hc, cl: [1, 7, 0.9, 0] }));
    const { P, Sft } = extras(id, D, F, HFs, rig);
    hairStrands(id, D, rig, F, HFs, Sft);
    const faceTex = paintFace(D.face);
    const out = { body, head, hair, prop: P.build(), soft: Sft.build(), faceTex, rig, ms: 0 };
  ...
  function build(id) {
    ...
    if (id === 'nyx') {
      ...
      orb.position.set(-0.4, 2.1, 0.1); g.add(orb);
  ...
  CT.heroines = { build, bake, IDS: ['kaela', 'nyx', 'vesna'], uniforms: U, DEF };
})();
