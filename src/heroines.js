...
(function () {
  ...
  Field.prototype.mask = function (gm, fn, th, o) { // colour + relief mask; fn(x,y,z) > 0 inside
    o = o || {}; const M = { gm, fn, th, rp: o.rp || 0.009, col: o.col, pid: o.pid || 0, gloss: o.gloss || 0, pri: o.pri || 0 };
  ...
  function polygonize(F, bnd, h, smoothIt, nSmooth) {
    ...
    for (let it = 0; it < (nSmooth === undefined ? 3 : nSmooth); it++) {
      const tn = new Float32Array(nor.length);
      for (let v = 0; v < nv; v++) {
        let ax = nor[v * 3] * 2, ay = nor[v * 3 + 1] * 2, az = nor[v * 3 + 2] * 2;
        for (const u of nb[v]) { ax += nor[u * 3]; ay += nor[u * 3 + 1]; az += nor[u * 3 + 2]; }
        const l = Math.hypot(ax, ay, az) || 1; tn[v * 3] = ax / l; tn[v * 3 + 1] = ay / l; tn[v * 3 + 2] = az / l;
      }
      nor.set(tn);
    }
  ...
  const PATTERN = `
    ...
    vec3 ctPat(vec3 c, float pid, vec3 o, float sd){
      ...
      } else if (pid < 7.5) {                  // hair: strand groups with dark partings and light strands
        float a = atan(o.x - uHC.x, o.z - uHC.z);
        float u = a * 9.0 + o.y * 2.2 + ctN(vec2(a * 3.0, o.y * 2.5)) * 2.2 + ctN(vec2(a * 12.0, o.y * 6.0)) * 0.6;
        float st = abs(fract(u) - 0.5) * 2.0;
        c *= mix(1.45, 0.42, smoothstep(0.15, 0.9, st)) * (0.85 + 0.3 * ctN(vec2(a * 40.0, o.y * 8.0)));
      } else if (pid < 8.5) {                  // wood
        c *= 0.75 + 0.4*ctN(vec2(o.x*300.0 + o.z*300.0, o.y*10.0));
      }
      else if (pid > 8.5 && pid < 9.5) {       // flag canton with star dots
        vec2 q = vec2(o.x * 46.0, o.y * 46.0); q.x += 0.5 * floor(q.y);
        c = mix(c, vec3(0.85, 0.82, 0.74), 1.0 - smoothstep(0.14, 0.22, length(fract(q) - 0.5)));
      }
      if (pid > 0.5 && pid < 6.5 && sd < 0.0035) c *= 0.5;   // hem / plate edge
      if ((pid > 0.5 && pid < 1.5 || pid > 3.5 && pid < 4.5) && sd > 0.0035 && sd < 0.0075) c *= 1.7;   // bronze rim
      if (pid > 1.5 && pid < 2.5 && sd > 0.0048 && sd < 0.0072 && fract((o.x + o.y + o.z) * 140.0) < 0.55) c = c * 1.9 + 0.03; // stitching
      return c;
    }`;
    ...
    function toon(kind) {
      ...
      m.onBeforeCompile = sh => {
        ...
        sh.vertexShader = sh.vertexShader
          .replace('#include <common>', '#include <common>\n' + IDLE + '\nattribute vec3 aCol1; attribute vec4 aCl; attribute vec3 aCol2; attribute vec4 aOv; varying vec3 vCol1; varying vec4 vCl; varying vec3 vCol2; varying vec4 vOv; varying vec3 vObj;')
          .replace('#include <begin_vertex>', 'vec3 transformed = ctIdle(position, aCl.w); vObj = position; vCol1 = aCol1; vCl = aCl; vCol2 = aCol2; vOv = aOv;');
        sh.fragmentShader = sh.fragmentShader
          .replace('#include <common>', '#include <common>\nuniform vec3 uRimCol; uniform float uRimK; uniform vec3 uWarm; uniform vec3 uHC; uniform float uSkinGloss;\nvarying vec3 vCol1; varying vec4 vCl; varying vec3 vCol2; varying vec4 vOv; varying vec3 vObj;\n' + PATTERN)
          .replace('#include <map_fragment>', '')
          .replace('#include <color_fragment>', `
            vec3 cBase = vColor.rgb; float gloss = uSkinGloss * ${kind === 'body' ? 'vOv.w' : kind === 'head' ? '0.5' : '1.0'};
            ${kind === 'head' ? 'vec4 fx = texture2D(map, vMapUv); cBase = mix(cBase, fx.rgb, fx.a * vCol1.x); gloss *= 1.0 - fx.a * vCol1.x;' : `
            { float aa = max(fwidth(vCl.x), 1e-5) * 0.8, m = vCl.x > -0.9 ? smoothstep(-aa, aa, vCl.x) : 0.0;
              if (m > 0.0) { cBase = mix(cBase, ctPat(vCol1, vCl.y, vObj, vCl.x), m); gloss = mix(gloss, vCl.z, m); }
              float ab = max(fwidth(vOv.x), 1e-5) * 0.8, m2 = vOv.x > -0.9 ? smoothstep(-ab, ab, vOv.x) : 0.0;
              if (m2 > 0.0) { cBase = mix(cBase, ctPat(vCol2, vOv.y, vObj, vOv.x), m2); gloss = mix(gloss, vOv.z, m2); }
              ctSkin *= 1.0 - max(m, m2); }`}
            diffuseColor.rgb = cBase;`)
          .replace('#include <gradientmap_pars_fragment>', `#include <gradientmap_pars_fragment>
            float ctCap = 0.0; float ctSkin = ${kind === 'body' || kind === 'head' ? '1.0' : '0.0'};
            vec3 ctGrad(vec3 n, vec3 l) { float d = dot(n, l); vec2 c = vec2(min(d * 0.5 + 0.5, mix(1.0, 0.74, ctCap)), 0.0); float g = texture2D(gradientMap, c).r;
              return vec3(g) * mix(vec3(1.0), mix(vec3(1.22, 0.8, 0.72), vec3(1.0), smoothstep(0.45, 0.95, g)), ctSkin); }`)
          .replace('#include <lights_toon_pars_fragment>', ShaderChunk_toon)
          .replace('#include <normal_fragment_maps>', `#include <normal_fragment_maps>
            if (vCl.x > 0.0 && vCl.y > 0.5 && vCl.y < 5.5 && vObj.y > 1.1 && ctSkin < 0.5) ctCap = 1.0;
          ...
          .replace('#include <opaque_fragment>', `
            { vec3 N = normalize(normal); vec3 V = normalize(vViewPosition); if (dot(N, V) < 0.0) N = -N; float ndv = clamp(dot(N, V), 0.0, 1.0);
                ...
                float sp = smoothstep(0.976, 0.982, nh) * 0.65 * (1.0 - ctCap);
  ...
  function geoFrom(pos, nor, idx, col0, col1, cl, uv, col2, ov) {
    const g = new T.BufferGeometry(), nv = pos.length / 3;
    if (!col2) col2 = new Float32Array(nv * 3);
    if (!ov) { ov = new Float32Array(nv * 4); for (let i = 0; i < nv; i++) ov[i * 4] = -1; }
    g.setAttribute('aCol2', new T.BufferAttribute(col2, 3));
    g.setAttribute('aOv', new T.BufferAttribute(ov, 4));
  ...
  function bodyField(rig, S) {
    ...
    F.ell(0, chs, [0, -0.075, 0.07], [0.034, 0.03, 0.03], 0.045);                            // sternum bridge (no V at the lower cleavage)
    F.ell(0, chs, [0, 0.112, -0.014], [0.122, 0.062, 0.078], 0.06);                          // shoulder girdle
    F.cone(0, W(chs, [0, 0.14, -0.018]), W(rig.head, [0, -0.062, -0.024]), 0.056, 0.046, 0.075); // neck
  ...
  // Bold Frazetta face paint: it must survive 640x360 + dither with the face 12 to 20 px tall.
  function paintFace(S) {
    const cv = document.createElement('canvas'); cv.width = cv.height = 1024; const g = cv.getContext('2d'); g.scale(2, 2);
    const rgrad = (x, y, r, col) => { const q = g.createRadialGradient(FU(x), FV(y), 1, FU(x), FV(y), r); q.addColorStop(0, col); q.addColorStop(1, 'rgba(0,0,0,0)'); g.fillStyle = q; g.fillRect(0, 0, 512, 512); };
    const ell = (x, y, rx, ry, rot, col) => { g.fillStyle = col; g.beginPath(); g.ellipse(FU(x), FV(y), rx, ry, rot || 0, 0, PI * 2); g.fill(); };
    // hairline shadow + temples
    { const q = g.createLinearGradient(0, FV(0.085), 0, FV(0.05)); q.addColorStop(0, 'rgba(40,14,10,0.75)'); q.addColorStop(1, 'rgba(40,14,10,0)'); g.fillStyle = q; g.fillRect(0, FV(0.1), 512, FV(0.05) - FV(0.1)); }
    for (const s of [-1, 1]) {
      rgrad(s * 0.06, 0.03, 60, 'rgba(60,20,14,0.45)');                                     // temple
      rgrad(s * 0.031, 0.008, 70, S.lid);                                                    // smoky eye socket
      rgrad(s * 0.047, -0.03, 60, S.blush);                                                  // cheek colour
      // cheekbone shadow: a diagonal band under the cheekbone, ear -> mouth corner
      g.save(); g.translate(FU(s * 0.05), FV(-0.037)); g.rotate(s * 0.55); g.scale(1, 0.3);
      { const q = g.createRadialGradient(0, 0, 2, 0, 0, 36); q.addColorStop(0, 'rgba(70,24,16,0.45)'); q.addColorStop(1, 'rgba(70,24,16,0)'); g.fillStyle = q; g.beginPath(); g.arc(0, 0, 36, 0, PI * 2); g.fill(); } g.restore();
      // jaw edge shadow
      rgrad(s * 0.058, -0.075, 50, 'rgba(60,18,12,0.5)');
    }
    rgrad(0, -0.098, 60, 'rgba(60,18,12,0.55)');                                             // under the chin
    ...
    for (const s of [-1, 1]) {
      ...
      // upper liner + lashes + wing (thick: 1 to 2 screen px at 2.5 m)
      g.fillStyle = '#070304'; g.beginPath(); g.moveTo(-w - 6, 6); g.quadraticCurveTo(-w * 0.15, -hgt * 2.1, w + 4, -hgt * 0.5);
      g.lineTo(w + 26, -hgt * 1.5); g.lineTo(w + 6, hgt * 0.1); g.quadraticCurveTo(-w * 0.15, -hgt * 1.15, -w, 5); g.fill();
      if (S.lidLow) { g.fillStyle = S.lidCol || '#3a2030'; g.beginPath(); g.moveTo(-w - 2, 3); g.quadraticCurveTo(-w * 0.2, -hgt * 1.9, w + 3, -hgt * 0.45); g.quadraticCurveTo(0, -hgt * 1.7 * (1 - S.lidLow), -w - 2, 3); g.fill();
        g.fillStyle = '#070304'; g.beginPath(); g.moveTo(-w - 2, 3); g.quadraticCurveTo(0, -hgt * 1.7 * (1 - S.lidLow) - 7, w + 5, -hgt * 0.5); g.lineTo(w + 24, -hgt * 1.3); g.lineTo(w + 4, 0); g.quadraticCurveTo(0, -hgt * 1.7 * (1 - S.lidLow) + 3, -w - 2, 6); g.fill(); }
      // lower liner
      g.strokeStyle = '#1a0a0a'; g.lineWidth = 4; g.beginPath(); g.moveTo(-w * 0.75, 6); g.quadraticCurveTo(0, hgt * 1.35, w, -hgt * 0.15); g.stroke();
      // catchlight: big and bright so one pixel survives
      g.fillStyle = '#ffffff'; g.beginPath(); g.arc(-s * 2 + 6, -8, 4.5, 0, PI * 2); g.fill();
      g.restore();
      // brows: strong and dark
      g.save(); g.translate(FU(s * 0.032), FV(0.027 + (S.browY || 0))); g.scale(-s, 1);
      g.fillStyle = S.brow; g.beginPath(); const ang = S.browAng || 0, arch = S.browArch || 0;
      g.moveTo(48, 6 + ang * 16); g.quadraticCurveTo(0, -22 - arch * 12, -48, -2); g.lineTo(-44, 14); g.quadraticCurveTo(0, -2 - arch * 12, 44, 24 + ang * 16); g.fill();
      g.restore();
    }
    // nose: side shadows, a bridge highlight, dark nostrils, the shadow under the tip
    g.fillStyle = 'rgba(80,26,16,0.35)';
    for (const s of [-1, 1]) { g.beginPath(); g.moveTo(FU(s * 0.006), FV(0.012)); g.quadraticCurveTo(FU(s * 0.013), FV(-0.012), FU(s * 0.016), FV(-0.03)); g.lineTo(FU(s * 0.009), FV(-0.03)); g.quadraticCurveTo(FU(s * 0.008), FV(-0.01), FU(s * 0.003), FV(0.012)); g.fill(); }
    { const q = g.createLinearGradient(FU(-0.004), 0, FU(0.004), 0); q.addColorStop(0, 'rgba(255,236,220,0)'); q.addColorStop(0.5, 'rgba(255,236,220,0.55)'); q.addColorStop(1, 'rgba(255,236,220,0)'); g.fillStyle = q; g.beginPath(); g.moveTo(FU(-0.002), FV(0.008)); g.lineTo(FU(0.002), FV(0.008)); g.lineTo(FU(0.004), FV(-0.022)); g.lineTo(FU(-0.004), FV(-0.022)); g.fill(); }
    ell(0.0, -0.034, 16, 5, 0, 'rgba(70,20,14,0.55)');
    for (const s of [-1, 1]) ell(s * 0.008, -0.031, 5, 3.5, 0, 'rgba(30,8,6,0.85)');
    // lips: full, dark line, bright highlight on the lower lip, shadowed corners
    const ly = FV(-0.048), lx = FU(0), lw = 64 * (S.mouthW || 1), grin = S.grin || 0, smirk = S.smirk || 0;
    g.fillStyle = S.lips; g.beginPath();
    g.moveTo(lx - lw, ly - grin * 10); g.quadraticCurveTo(lx - lw * 0.45, ly - 26, lx - 9, ly - 20); g.quadraticCurveTo(lx, ly - 15, lx + 9, ly - 20);
    g.quadraticCurveTo(lx + lw * 0.45, ly - 26, lx + lw, ly - grin * 10 - smirk * 10);
    g.quadraticCurveTo(lx + lw * 0.45, ly + 38 - grin * 6, lx, ly + 39 - grin * 4); g.quadraticCurveTo(lx - lw * 0.45, ly + 38 - grin * 6, lx - lw, ly - grin * 10); g.fill();
    g.strokeStyle = S.lipLine || '#2a0406'; g.lineWidth = 5; g.beginPath(); g.moveTo(lx - lw * 0.95, ly - grin * 10); g.quadraticCurveTo(lx, ly + 3 + grin * 10, lx + lw * 0.95, ly - grin * 10 - smirk * 10); g.stroke();
    if (S.teeth) { g.fillStyle = '#f2ebe2'; g.beginPath(); g.moveTo(lx - lw * 0.7, ly - grin * 6); g.quadraticCurveTo(lx, ly + 13, lx + lw * 0.7, ly - grin * 6 - smirk * 6); g.quadraticCurveTo(lx, ly + 2, lx - lw * 0.7, ly - grin * 6); g.fill(); }
    ell(0.006, -0.057, 20, 6, 0, 'rgba(255,240,235,0.75)');                                  // lower-lip highlight
    for (const s of [-1, 1]) ell(s * 0.033, -0.049, 4, 3, 0, 'rgba(40,10,8,0.45)');           // mouth corners
    ell(0, -0.069, 22, 7, 0, 'rgba(70,22,14,0.45)');                                         // shadow under the lower lip
    if (S.freckles) { let sd = 7; const rnd = () => { sd = (sd * 16807) % 2147483647; return sd / 2147483647; };
      g.fillStyle = 'rgba(110,40,16,0.7)'; for (let i = 0; i < 80; i++) { const a = rnd() * PI * 2, r = Math.sqrt(rnd()); g.beginPath(); g.arc(FU(Math.cos(a) * 0.055 * r), FV(-0.018 + Math.sin(a) * 0.022 * r), 2 + rnd() * 2, 0, PI * 2); g.fill(); } }
    if (S.paint) { g.fillStyle = S.paint; const s = S.paintSide || -1; for (let i = 0; i < 3; i++) { g.beginPath(); const x0 = FU(s * 0.028), y0 = FV(-0.016 - i * 0.012); g.moveTo(x0, y0); g.lineTo(FU(s * 0.07), y0 - 9); g.lineTo(FU(s * 0.07), y0 + 3); g.lineTo(x0, y0 + 9); g.fill(); } }
    if (S.scar) { g.strokeStyle = 'rgba(150,50,50,0.9)'; g.lineWidth = 4; g.beginPath(); g.moveTo(FU(0.022), FV(0.047)); g.lineTo(FU(0.04), FV(0.014)); g.stroke(); }
  ...
  DEF.kaela = {
    ...
    pose: {
      ...
      hYaw: 0.05, hPitch: -0.02, hRoll: -0.06,
  ...
  DEF.vesna = {
    ...
    pose: {
      ...
      hYaw: -1.48, hPitch: 0.04, hRoll: 0.08,
  ...
  function outfit(id, F, rig, D) {
    ...
    if (id === 'kaela') {
      ...
      for (const s of [-1, 1]) {
        F.mask(TOR, cupMask(s, 0.097, 0.06), 0.005, { col: bronze, pid: 1, gloss: 0.7, rp: 0.012 });
        F.mask(TOR, MK.cap(W(chs, [s * 0.125, 0.075, 0.06]), W(chs, [s * 0.1, 0.19, -0.02]), 0.011), 0.004, { col: hx('#6a4012'), pid: 2, gloss: 0.3, pri: 1 });
      }
      F.mask(TOR, MK.band(chs, q => q[1] + 0.078 - 0.02 * q[2], 0.011), 0.004, { col: hx('#6a4012'), pid: 2, gloss: 0.3, pri: 1 });
      // belt: low slung at front
      F.mask(TOR | LEGS, MK.and(MK.band(pel, q => q[1] - (0.075 - 0.3 * q[2]), 0.026), MK.half(pel, q => 0.3 - Math.abs(q[0]))), 0.01, { col: hx('#3e2212'), pid: 2, gloss: 0.35, pri: 1 });
    ...
    if (id === 'nyx') {
      ...
      for (const s of [-1, 1]) {
        ...
        F.mask(TOR, MK.cap(W(chs, [s * 0.042, 0.17, 0.035]), W(chs, [s * 0.02, 0.215, -0.04]), 0.013), 0.005, { col: indigo, pid: 5, gloss: 0.12, pri: 1 });
      }
      ...
      F.mask(TOR | LEGS, MK.and(MK.band(pel, q => q[1] - (0.07 - 0.25 * q[2]), 0.014), MK.half(pel, q => 0.3 - Math.abs(q[0]))), 0.007, { col: silver, pid: 4, gloss: 1.3, pri: 1 });
    ...
    if (id === 'selene') {
      ...
      F.mask(TOR, MK.and(tee0, canton), 0, { col: hx('#28386c'), pid: 9, gloss: 0.1, pri: 3 });
      for (const s of [-1, 1]) {
    ...
    if (id === 'vesna') {
      ...
      for (const s of [-1, 1]) {
        ...
        F.mask(TOR, MK.cap(W(chs, [s * 0.125, 0.075, 0.06]), W(chs, [s * 0.08, 0.19, -0.03]), 0.01), 0.004, { col: leather, pid: 2, gloss: 0.3, pri: 1 });
      }
      F.mask(TOR, MK.band(chs, q => q[1] + 0.075 - 0.02 * q[2], 0.01), 0.004, { col: leather, pid: 2, gloss: 0.3, pri: 1 });
      F.mask(TOR | LEGS, MK.and(MK.band(pel, q => q[1] - (0.06 - 0.28 * q[2]), 0.017), MK.half(pel, q => 0.3 - Math.abs(q[0]))), 0.009, { col: leather, pid: 2, gloss: 0.4, pri: 1 });
      ...
      for (const s of [-1, 1]) {
        ...
        F.mask(LEGS, MK.band(fr, q => q[1] - (lk - 0.055), 0.01), 0.005, { col: leather, pid: 2, gloss: 0.4, pri: 1 });
        F.mask(LEGS, MK.and(MK.band(L.foot, q => q[2] - 0.05 - q[1] * 0.3, 0.008), MK.half(L.foot, q => q[1] + 0.05)), 0.004, { col: leather, pid: 2, gloss: 0.4, pri: 1 });
        const A = rig.arm[s], ua = nrm(sub(A.elb, A.wr)), fa = { o: A.wr, R: basisY(ua, [0, 0, 1]) };
  ...
  // and the zones where the oiled-skin highlight may appear. Every point sits on the real surface (surfPt).
  function anatomy(F, rig, D) {
    const { pel, wst, chs } = rig, sh = [], sp = [], ath = D.athletic === undefined ? 1 : D.athletic;
    const S = (o, d) => surfPt(F, o, d);
    const fC = WD(chs, [0, 0, 1]), uC = WD(chs, [0, 1, 0]), fW = WD(wst, [0, 0, 1]), fP = WD(pel, [0, 0, 1]);
    const line = (a, b, r, k) => sh.push({ a, b: b || a, r: r * 1.45, k: k * 1.5 });
    const zone = (a, b, r, k) => sp.push({ a, b: b || a, r, k: k || 1 });
    line(S(W(chs, [0, 0.168, 0]), fC), null, 0.013, 0.45);                                   // sternal notch
    line(S(W(chs, [0, 0.07, 0]), fC), S(W(chs, [0, -0.035, 0]), fC), 0.014, 0.4);          // sternum / cleavage
    line(S(W(wst, [0, -0.028, 0]), fW), null, 0.011, 0.6);                                   // navel
    line(S(W(wst, [0, -0.07, 0]), fW), S(W(chs, [0, -0.12, 0]), fC), 0.008, 0.3 * ath);     // linea alba
    for (const y of [0.045, 0.0]) line(S(W(wst, [-0.04, y, 0]), fW), S(W(wst, [0.04, y, 0]), fW), 0.006, 0.16 * ath); // abs
    line(S(W(wst, [0, -0.06, 0]), scl(fW, -1)), S(W(chs, [0, 0.12, 0]), scl(fC, -1)), 0.011, 0.32); // spine groove
    for (const s of [-1, 1]) {
      const sC = WD(chs, [s, 0, 0]), sW = WD(wst, [s, 0, 0]), sP = WD(pel, [s, 0, 0]);
      line(S(W(chs, [s * 0.03, 0.15, 0]), fC), S(W(chs, [s * 0.13, 0.158, -0.01]), add(fC, scl(uC, 0.3))), 0.012, 0.35);   // under the collarbone
      line(S(W(chs, [s * 0.035, 0.172, 0]), fC), S(W(chs, [s * 0.13, 0.18, -0.01]), add(fC, uC)), 0.009, -0.3);           // collarbone ridge
      zone(S(W(chs, [s * 0.035, 0.172, 0]), fC), S(W(chs, [s * 0.13, 0.18, -0.01]), add(fC, uC)), 0.025);
      line(S(W(chs, [s * 0.04, -0.1, 0]), fC), S(W(chs, [s * 0.12, -0.085, 0]), add(scl(fC, 0.6), scl(sC, 0.8))), 0.013, 0.32); // under-bust fold
      line(S(W(chs, [s * 0.08, -0.09, 0]), add(sC, scl(fC, 0.4))), S(W(chs, [s * 0.09, -0.13, 0]), add(sC, scl(fC, 0.4))), 0.007, 0.2 * ath); // ribs
      line(S(W(wst, [s * 0.07, -0.04, 0]), add(scl(fW, 0.7), scl(sW, 0.7))), S(W(pel, [s * 0.035, -0.03, 0]), fP), 0.009, 0.35); // obliques-to-hip V
      line(S(W(pel, [s * 0.09, 0.1, 0]), add(fP, scl(sP, 0.3))), null, 0.016, -0.25);    // hip bone
      line(S(W(pel, [s * 0.035, 0.1, 0]), scl(fP, -1)), null, 0.012, 0.5);                // back dimple
      line(S(W(pel, [s * 0.03, -0.14, -0.03]), scl(fP, -1)), S(W(pel, [s * 0.11, -0.12, -0.02]), scl(fP, -1)), 0.014, 0.3); // gluteal fold
      zone(S(W(chs, [s * 0.074, 0.06, 0]), add(fC, scl(uC, 0.3))), null, 0.035);           // top of the bust
      zone(S(W(pel, [s * 0.13, 0.0, 0]), sP), null, 0.08);                                 // hip
      zone(S(W(pel, [s * 0.07, -0.02, 0]), scl(fP, -1)), null, 0.06);                      // glute
      // arm
      const A = rig.arm[s], out = WD(chs, [s, 0, 0]);
      line(S(lerp3(A.elb, A.sh, 0.6), add(scl(fC, 0.9), scl(out, 0.4))), S(lerp3(A.elb, A.sh, 0.8), out), 0.009, 0.35);  // deltoid / biceps
      line(S(A.elb, scl(fC, -1)), null, 0.012, 0.25);                                       // elbow
      zone(S(add(A.sh, scl(uC, 0.02)), add(out, uC)), null, 0.06);                          // shoulder cap
      // leg
      const L = rig.leg[s], H = L.hip, K = L.knee, An = L.ank, uL = nrm(sub(H, K)), f0 = WD(L.foot, [0, 0, 1]);
      const fL = nrm(sub(f0, scl(uL, dot(f0, uL)))), oL = scl(nrm(cross(uL, fL)), s);
      line(S(sub(K, scl(uL, 0.04)), fL), null, 0.016, 0.35);                                // under the kneecap
      line(S(add(K, scl(uL, 0.008)), fL), null, 0.015, -0.3);                               // kneecap
      line(S(lerp3(An, K, 0.25), sub(fL, scl(oL, 0.3))), S(lerp3(An, K, 0.8), sub(fL, scl(oL, 0.3))), 0.008, -0.25); // shin ridge
      line(S(lerp3(An, K, 0.45), add(scl(fL, -1), scl(oL, 0.6))), S(lerp3(An, K, 0.85), add(scl(fL, -1), scl(oL, 0.7))), 0.009, 0.3); // calf line
      line(S(lerp3(K, H, 0.12), sub(scl(fL, 0.5), scl(oL, 0.9))), null, 0.02, 0.18);         // inner knee
      zone(S(lerp3(K, H, 0.3), add(fL, scl(oL, 0.5))), S(lerp3(K, H, 0.8), add(fL, scl(oL, 0.5))), 0.05);   // thigh
      zone(S(lerp3(An, K, 0.3), fL), S(lerp3(An, K, 0.8), fL), 0.03);                        // shin
    }
    const segD = (x, y, z, f) => { const ba = sub(f.b, f.a), pa = [x - f.a[0], y - f.a[1], z - f.a[2]], l2 = dot(ba, ba); const t = l2 > 1e-9 ? clamp(dot(pa, ba) / l2, 0, 1) : 0; return len(sub(pa, scl(ba, t))); };
    return {
      shade(x, y, z, c) {
        let dk = 0, lt = 0;
        for (const f of sh) { const d = segD(x, y, z, f); if (d >= f.r) continue; const w = Math.abs(f.k) * sstep(f.r, 0, d); if (f.k > 0) dk = Math.max(dk, w); else lt = Math.max(lt, w); }
        if (dk > 0) c = mixc(c, [c[0] * 0.5, c[1] * 0.32, c[2] * 0.3], Math.min(dk, 0.85));
        if (lt > 0) c = mixc(c, [Math.min(c[0] * 1.4 + 0.03, 1), Math.min(c[1] * 1.4 + 0.03, 1), Math.min(c[2] * 1.35 + 0.03, 1)], Math.min(lt, 0.8));
        return c;
      },
      spec(x, y, z) { let m = 0.12; for (const f of sp) { const d = segD(x, y, z, f); if (d < f.r) m = Math.max(m, f.k * sstep(f.r, f.r * 0.35, d)); } return m; },
    };
  }
  function bakeBody(F, rig, D, h) {
    const bnd = F.bounds(0.035); bnd[1] = Math.max(bnd[1], -0.02);
    const pg = polygonize(F, bnd, h, 3, 4);
    const nv = pg.nv, c0 = new Float32Array(nv * 3), c1 = new Float32Array(nv * 3), cl = new Float32Array(nv * 4), c2 = new Float32Array(nv * 3), ov = new Float32Array(nv * 4);
    const skin = hx(D.skin), knee = mixc(skin, hx('#c05a48'), 0.25), AN = anatomy(F, rig, D);
    for (let v = 0; v < nv; v++) {
      ...
      // two slots: the base garment (pri 0, the largest signed distance) and one overlay (trim, print, buckle, paint)
      let base = null, bsd = -1e9, ovl = null, osd = -1e9, osc = -1e9;
      for (const M of F.masks) {
        if (!(M.gm & (1 << gi))) continue; const sd = M.fn(x, y, z);
        if (!M.pri) { if (sd > bsd) { bsd = sd; base = M; } continue; }
        const sc = sd > 0 ? 10 + M.pri * 10 + sd : sd; if (sc > osc) { osc = sc; ovl = M; osd = sd; }
      }
      let c = skin;
      ...
      c = AN.shade(x, y, z, c);
      c0.set(c, v * 3);
      if (base) { c1.set(base.col, v * 3); cl[v * 4] = clamp(bsd, -0.05, 0.05); cl[v * 4 + 1] = base.pid; cl[v * 4 + 2] = base.gloss; }
      else { c1.set(c, v * 3); cl[v * 4] = -1; }
      if (ovl) { c2.set(ovl.col, v * 3); ov[v * 4] = clamp(osd, -0.05, 0.05); ov[v * 4 + 1] = ovl.pid; ov[v * 4 + 2] = ovl.gloss; }
      else ov[v * 4] = -1;
      ov[v * 4 + 3] = AN.spec(x, y, z);
    }
    return geoFrom(pg.pos, pg.nor, pg.idx, c0, c1, cl, null, c2, ov);
  }
  ...
  function bake(id) {
    ...
    const head = bakeLocal(HFs, rig.head, 0.0061, (l, n) => ({ c0: mixc(skin, [skin[0] * 0.55, skin[1] * 0.38, skin[2] * 0.36], clamp(sstep(-0.2, -0.75, n[1]) * sstep(-0.03, -0.07, l[1]) + 0.35 * sstep(0.55, 0.95, Math.abs(n[0])) * sstep(0.02, -0.04, l[1]), 0, 0.8)), c1: [sstep(0.0, 0.3, n[2]) * sstep(-0.13, -0.1, l[1]), 0, 0], cl: [-1, 0, 0, 0] }), l => [0.5 + l[0] / 0.2, 0.5 + (l[1] + 0.02) / 0.2]); mark('head');
